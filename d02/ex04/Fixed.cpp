...
/*   Updated: 2018/06/21 23:07:53 by vbrazas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	this->_val = (int)roundf( (this->toFloat() + toAdd.toFloat()) * (1 << Fixed::_fractionalBits) );
}

void			Fixed::operator-=( const Fixed &toSubtract )
{
	this->_val = (int)roundf( (this->toFloat() - toSubtract.toFloat()) * (1 << Fixed::_fractionalBits) );
}

void			Fixed::operator*=( const Fixed &toMultiply )
{
	this->_val = (int)roundf( (this->toFloat() * toMultiply.toFloat()) * (1 << Fixed::_fractionalBits) );
}

void			Fixed::operator/=( const Fixed &toDivide )
{
	this->_val = (int)roundf( (this->toFloat() / toDivide.toFloat()) * (1 << Fixed::_fractionalBits) );
}
